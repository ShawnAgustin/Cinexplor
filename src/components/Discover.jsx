...
function Discover(props) {
		...
		let responseM = await axios
			...
			.then((res) =>
				res.data.results.map((item) => ({
					...
					out: item.release_date || 'N/A ',
					media_type: 'movie',
				}))
			);

		let responseT = await axios
	...
	useEffect(() => {
		...
		axios
			.get(
				'https://api.themoviedb.org/3/watch/providers/movie?api_key=6599bc26f4ca86fd26961ad8384590da&language=en-US&watch_region=US'
			)
			.then((res) =>
				setProvInfo(
			...
			<div className='selected'>
				{
					...
					providers.map((element) => {
						...
						let selected = provInfo.filter((item) => {
							return item.provider_id === element;
						});
						try {
							return (
								<div className='icon-container' onClick={() => handleLogoClick(element)}>
									...
									<img
										key={element}
										className='icon'
										src={img_URL + selected[0].logo_path}
										alt={selected[0].name}
									/>
								</div>
							);
				...
				<div className='sort'>
					<div className='options-bar'>
						...
						<div className='search-type'>
							<li
								className={value1[0] !== 1910 || value1[1] !== 2022 ? 'active1' : 'inactive1'}
								onClick={() => handleOpen('year')}>
								Released Year <BsFillCaretDownFill />
							</li>
							{yearOpen && (
								<div className='release-year'>
									<div>
										<div
											...
											<p
												className={
													value1[0] !== 1910 || value1[1] !== 2022 ? 'active' : 'inactive'
												}>
												{value1[0]}-{value1[1]}
											</p>{' '}
											<span
							...
							<li
								className={
									activeTVGenres.length > 0 || activeMovieGenres.length > 0
										? 'active1'
										: 'inactive1'
								}
								onClick={() => handleOpen('genre')}>
								Genre <BsFillCaretDownFill />
							</li>
							{genresOpen && (
									...
									<div
										...
										{allGenres.map((item) => {
											return (
												<li
													className={
														activeTVGenres.includes(item[0]) || activeMovieGenres.includes(item[0])
															? 'active'
															: 'inactive'
													}
													onClick={() => handleSetGenre(item[0])}>
													{item[1]}
												</li>
											);
										})}
							...
							<li className={ageFilter !== 'NC-17' ? 'active1' : 'inactive1'}>
								<span
									style={{
										display: 'flex',
										alignItems: 'center',
									...
									Age Filter <BsFillCaretDownFill />
								</span>
								{ageOpen && (
									<div className='age'>
										<li onClick={() => setAgeFilter('NC-17')}>
											No filter {ageFilter === 'NC-17' && <img src={check2} alt='check' />}
										</li>
										<li onClick={() => setAgeFilter('PG')}>
											Suitable for Kids {ageFilter === 'PG' && <img src={check2} alt='check' />}
										</li>
										<li onClick={() => setAgeFilter('PG-13')}>
											Suitable for Teens {ageFilter === 'PG-13' && <img src={check2} alt='check' />}
										</li>
										<li onClick={() => setAgeFilter('R')}>
											Mature (15+) {ageFilter === 'R' && <img src={check2} alt='check' />}
										</li>
										<span
											style={{
												fontSize: '0.8em',
